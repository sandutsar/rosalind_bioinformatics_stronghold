...
def subs(s, t, save=False, path=None, filename='rosalind_subs_1_output', ext='txt'):
    ...
    # assert is_DNA(t), f'Error: Your string is not a valid DNA string! \
    #     It must be composed using {DNA_ALPHABET} alphabet!'
    ...
    if not isinstance(t, str):
        raise TypeError(f'Error: type(s) = {type(t).__name__} must be str!')
    if not is_DNA(t):
        raise StringIsNotDNAError()
    if len(t) > 1e3:
...
if __name__ == '__main__':
    if len(sys.argv) == 3:
        ...
        with open(os.path.join(os.path.dirname(os.path.realpath(__file__)), \
                            'rosalind_subs_1_dataset.txt'), 'r') as file:
            s = file.readline()[:-1]
            t = file.readline()[:-1]

    print(s, t)

    print(*subs(s, t, save=True))
